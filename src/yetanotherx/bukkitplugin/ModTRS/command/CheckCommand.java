...
public class CheckCommand implements CommandExecutor {
	...
	if( !ModTRSPermissions.has(player, "modtrs.command.check") ) {
		...
		ArrayList<ModTRSRequest>requests = ModTRSRequestTable.getOpenRequests(parent, type);
		...
		if( type.equals("open") && page == 1 && !ModTRSSettings.databases.isEmpty() ) {
			...
			PreparedStatement prep = tempConn.prepareStatement(parent.databaseHandler.getDatabase().getOpenRequests() + " WHERE request_status = 0 OR request_status = 1");
		    ...
		    dbCounts = dbCounts.substring(0, dbCounts.length() - 2);
                    ...
                    //This is broken. It needs to be re-thought and done right
